...
ggplot(data = hers, mapping=aes(y=HDL, x=BMI, color=agec)) +
  labs(x="Centered BodyMassIndex", y= "HDL", title = "LOESS and lm") + 
  ...
  ggPredict(HDL_model_plot, interactive= TRUE)
```
...
ggplot(data = hers, mapping=aes(x=BMI, y=LDL)) +
  ...
  geom_point(shape=20, color=2, na.rm=TRUE) + 
  geom_smooth(method = "loess", na.rm=TRUE) +
  geom_smooth(method="lm", formula=y~x, color=1, linetype=2, ymax= 350, ymin=50, na.rm=TRUE)
...
ggplot(data = hers, mapping=aes(x=BMI, y=HDL)) + 
  ...
  geom_point(shape=20, color=2, na.rm=TRUE) + 
  geom_smooth(method="auto", formula=y~ns(x))
#
#
ggplot(data = hers, mapping=aes(x=BMI, y=LDL)) + 
  labs(x="Body Mass Index kg/m^2", y= "LDL Cholesteron mg/dL", title = "Natural Splines") + 
  geom_point(shape=20, color=2, na.rm=TRUE) + 
  geom_smooth(method="auto", formula=y~ns(x), na.rm=TRUE)
#
