...
```
From these results we see that the regression coefficients for fibrogen is significant at the 5% level. An inclrease of one unit in this variable increases le log-odds on favor of an ESR value greater then 20 by estimated 1.83 with 95% confidence interval:
...
```
These are the values of the odds themselves (by exponentiating the estimate). So **increased values of fibrogen lead to a grater probability of an ESR value greater than 20**.
...
To compare the two models (with fibrinogen and fibrinogen + gamma globulin)
```{r}
